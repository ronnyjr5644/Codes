...
class Solution {
  public:
    void solve2(struct Node *root,vector<int>&x1,int& i){
      if(root==nullptr){
          return;
      }
        
      solve2(root->left,x1,i);
      root->data=x1[i];
      i++;
      solve2(root->right,x1,i);
    }
  void solve(struct Node *root,vector<int>&x1){
      if(root==nullptr){
          return;
      }
      solve(root->left,x1);
      x1.push_back(root->data);
      solve(root->right,x1);
  }
